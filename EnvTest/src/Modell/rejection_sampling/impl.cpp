...
std::vector<double> rejection_sampler<modellType>::solve(simulation_params param)
{
	...
	std::for_each(raw_result_.begin(), raw_result_.end(), [this](sample_result& result) {
	...
	//get g(x) as const
	double L_max = std::max_element(raw_result_.begin(), raw_result_.end(), 
	...
	if (std::isinf(L_max))
	{
		...
		L_max = std::max_element( raw_result_.begin(), raw_result_.end(), 
			[](const sample_result& A, const sample_result& B) {return A.L < B.L; })->L;
	}
	//rejection sampling
	std::for_each(raw_result_.begin(), raw_result_.end(), [&U, &L_max](sample_result& A) {
		auto u = U();
		if (u < A.L / L_max)
			A.accepted = true;
		else
...
template<class modellType>
std::vector<double> rejection_sampler<modellType>::solve_p(simulation_params param)
{
	std::random_device seed;
	std::default_random_engine gen(seed());
	auto U = std::bind(std::uniform_real_distribution(0., 1.), gen);
	...
	//get estimation from result
	auto result = estimate_result_p_();
	return result;
}
...
//		filtered_result_.push_back(I);
	...
	avg = std::transform_reduce(filtered_result_.begin(), filtered_result_.end(), avg,
...
template<class modellType>
std::vector<double> rejection_sampler<modellType>::estimate_result_p_()
{
	if (raw_result_.empty())
		throw std::exception("Internal error: raw_result is empty.");
	auto example = *raw_result_.begin();
